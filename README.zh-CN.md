...
<p align="center">
  Travel Agent 是一个全栈旅行规划工作台，能够把自然语言需求和旅行截图转成带有结构化行程、地图增强、预算校验、执行时间线和旅行手账导出的可执行方案。
</p>
...
很多旅行助手只能返回一段聊天文本，这个仓库更关注完整的产品流程：

- 按请求类型路由到不同 specialist agent，而不是所有问题都走一个通用补全。
- 产出结构化行程，包含每日安排、预算区间、酒店建议和约束检查。
- 用高德 / Amap 做天气、POI、地理位置和路线增强。
- 在前端展示执行时间线，而不是把规划过程完全藏起来。
- 支持截图辅助输入和旅行手账导出，而不只是聊天回答。
...
- 左侧：历史会话
- 中间：聊天输入区，支持粘贴截图、拖拽图片、点击上传
- 右侧：结构化行程、地图、执行过程和旅行手账导出
- 页面整体：压缩成首屏工作台，避免长页面滚动

<p align="center">
  <img src="./docs/assets/homepage-zh-CN.png" alt="Travel Agent 工作台截图" />
</p>
...
| 反馈运营能力 | 支持推荐结果导出与离线分析脚本，用于后续产品调优 |
